...
autocmd("LspAttach", {
  ...
  callback = function(event)
    ...
    if client:supports_method(lsp.protocol.Methods.textDocument_documentHighlight) then
      ...
      autocmd("LspDetach", {
        ...
        callback = function(ev)
          ...
          api.nvim_clear_autocmds({ group = "lsp-highlight", buffer = ev.buf })
        end,
      })
    end
    ...
    if client:supports_method(lsp.protocol.Methods.textDocument_completion) then
      ...
      map("i", "<cr>", function()
        return fn.pumvisible() ~= 0 and "<C-y>" or "<cr>"
    ...
    if client.name == "clangd" then
      map("n", "<leader>ch", function() vim.cmd.LspClangdSwitchSourceHeader() end, o)
      map("n", "<leader>cs", function() vim.cmd.LspClangdShowSymbolInfo() end, o)
    end
  end,
})
