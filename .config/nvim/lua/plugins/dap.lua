...
local dap, dapview, dapvirtual = require("dap"), require("dap-view"), require("nvim-dap-virtual-text")
  ...
  dap.adapters = {
    ...
    lldb = {
      name = "lldb", type = "executable", command = "lldb-dap",
      options = { initialize_timeout_sec = 10 },
    },
    gdb = {
      name = "gdb", type = "executable", command = "gdb",
      args = { "--interpreter=dap", "--eval-command", "set print pretty on" },
    },
    cppdbg = {
      name = "cppdbg", id = "cppdbg", type = "executable",
  ...
  dap.configurations.sh = {
    {
      name = "[bashdb]", type = "bashdb", request = "launch",
      showDebuggerOutput = true, pathBashdb = "bashdb",
      pathBashdbLib = "/usr/share/bashdb/", trace = true,
      file = "${file}", program = "${file}", cwd = "${workspaceFolder}",
      pathCat = "cat", pathBash = "bash", pathMkfifo = "mkfifo", pathPkill = "pkill",
      args = get_args, env = {}, terminalKind = "integrated",
    },
  }
  ...
  local function log_probe_rs(message)
    ...
    if file then file:write(message); file:close() end
  end
